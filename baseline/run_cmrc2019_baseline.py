...
def main():
    ...
    if args.do_train:
        ...
        if os.path.exists("train_baseline.pkl") and False:
            train_examples = cPickle.load(open("train_baseline.pkl", mode='rb'))#这种方式耗费内存更大
        else:
    ...
    if args.do_train:
        ...
        try:
            with open(cached_train_features_file, "rb") as reader:
                train_features = pickle.load(reader)#很占有内存。虽然在磁盘上只有2.1GB,加载到内存后占用0.8*32GB
        except:
        ...
        # 此时已经占有0.8*32GB内存(如果加载dump文件)，如果是从原始数据加载而不是加载dump文件则内存占用为0.7*32GB。
        # pdb.set_trace()
